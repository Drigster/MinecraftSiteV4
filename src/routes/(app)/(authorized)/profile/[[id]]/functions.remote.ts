...
import { deleteCape, deleteSkin, saveSkin, sendChangeEmailEmail, sendChangePasswordEmail, sendVerificationEmail } from '$lib/util.server';
import { db } from '$lib/db';
import { setFlash } from 'sveltekit-flash-message/server';
import { capeSchema, skinSchema, usernameChangeSchema } from './schemas';
import fs from "node:fs";

export const removeSkin = form(async (data) => {
	const { locals, cookies } = getRequestEvent();
    
	if (locals.user == null) {
		redirect(302, "/login");
	} else if (!locals.user.verified) {
		redirect(302, "/notverified");
	}
...
export const removeCape = form(async (data) => {
	const { locals, cookies } = getRequestEvent();
...
export const changeUsername = form(async (data) => {
    ...
    const result = usernameChangeSchema.safeParse(Object.fromEntries(data.entries()));
    if(!result.success){
        setFlash({
            type: "error",
...
export const changeEmail = form(async () => {
    ...
    let user = await db.selectFrom("User").selectAll().where("id", "=", locals.user.id).executeTakeFirstOrThrow();
    ...
    if (await sendChangeEmailEmail(user)) {
        ...
        setFlash({
            type: "error",
            message: "Не удалось отправить сообщение на почту. Попробуйте позже."
        }, cookies);
    }
});
...
export const uploadSkin = form(async (data) => {
    ...
    const result = await skinSchema.safeParseAsync(Object.fromEntries(data.entries()));
    if(!result.success){
        setFlash({
            type: "error",
            message: result.error.issues[0].message
...
export const uploadCape = form(async (data) => {
    ...
    const result = await capeSchema.safeParseAsync(Object.fromEntries(data.entries()));
    if(!result.success){
        setFlash({
            type: "error",
            message: result.error.issues[0].message
